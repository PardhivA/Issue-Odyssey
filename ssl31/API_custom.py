...
def issue_issue_linker(data):
    ...
    for issue in issues:
        ...
        if data["issueNumber"] == int(cur_issue_data["issueNumber"]):
            found_issue = issue
            break
    ...
    related_issue_number = related_issues[str(found_issue["issue_index"]-1)]

    all_possibly_related_issue_numbers = []
    for temp in related_issues:
        issue_number, related_issue_number_of_this_issue = temp, related_issues[temp] 

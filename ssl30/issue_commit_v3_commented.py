...
import pydriller
import re
# This class interacts with the GitHub API to fetch issues and commits from a specified repository.
class GithubFetcher:
    ...
    # Method to rotate through API keys
    ...
    # Method to extract rate limit information from response headers
    ...
    # Method to make GET requests to the GitHub API
    def get(self, url, headers=None, params=None):
        if headers is None:
            headers = {}

        # Retry logic for handling rate limiting and network errors
    ...
    # Method to fetch issues from the GitHub API
    ...
    # Method to fetch commits from the repository
    ...
    # Method to save fetched issues to a file
    ...
    # Method to save fetched commits to a file
...
def compare_strings(str1, str2):
    ...
    lines1 = str1.splitlines() if str1 else []
    lines2 = str2.splitlines() if str2 else []
...
def issue_date_generator(issue, type):
    # Generates date from issue's created_at or closed_at field
...
def find_last_occurrence(text, substring):
    # Finds the last occurrence of a substring in a text
    last_pos = -1
...
def issue_commit_map_fun(issue, summarization):
    # Generates a response using a conversational AI model based on issue and commit summarization
    issue_title = issue["title"] if issue["title"] else ""
    issue_body = issue["body"] if issue["body"] else ""
    prompt = "Here is the info:\nIssue Title: " + issue_title + "\nIssue Body: " + issue_body + "\nSummarization: " + summarization
    messages=[{
                    'role': 'user',
...
for issue in issues:
    ...
    with open("issues_commit_summarization_individual_ilinker.json",'a') as f:
        f.write(json_string + '\n')       
...
import json
with open("issues_commit_summarization_ilinker.json",'a') as f:

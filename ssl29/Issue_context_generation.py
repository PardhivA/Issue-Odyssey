...
def issue_commit_map_fun(issue, summarization):
    messages=[{
                    ...
                    OUTPUT: The percentage of similarity for this issue and commit summarization is 100.
                    ...
                    OUTPUT: The percentage of similarity for this issue and commit summarization is 50.
                    ...
                    OUTPUT: The percentage of similarity for this issue and commit summarization is 0.
        ...
        Here is the info:
        issue_title:''' + " " if issue["title"] == None else issue["title"]+'''
        issue_body: ''' + " " if issue["body"] == None else issue["body"]+ '''
    ...
    response = tokenizer.decode(
        output["sequences"][0][len(encodeds[0]):], 
        skip_special_tokens=True
    )
    print(response)
    # substring = "The percentage of mapping of issue and commit summarisation is"
    # last_occurrence = find_last_occurrence(response, substring)
    # # last_occurrence_builtin = find_last_occurrence_builtin(text, substring)

    # if last_occurrence != -1:
    #     print(f"The last occurrence of '{substring}' (using iteration) is at index {last_occurrence}")

    print("response: ", response)
    temp = response.split("The percentage of mapping of issue and commit summarisation is ")
    print(temp[1])
    answer = temp[len(temp)- 1].split("%")
    return int(answer[0])
